...
var S={po:true,fm:false,cd:{id:'default',title:'',content:''}};
...
var R=function(){var ed=$('editor'),md=ed.value,h=RH(md);$('preview').innerHTML=h;try{var r=ed.scrollTop/(ed.scrollHeight-ed.clientHeight||1),pv=$('preview');pv.scrollTop=r*(pv.scrollHeight-pv.clientHeight)}catch(e){}US(md);SD()};
...
var TP=function(){S.po=!S.po;$('preview-col').classList.toggle('hidden',!S.po);$('preview-col').classList.toggle('visible',S.po)};
...
var EP=function(){var hc=$('preview').innerHTML,pw=window.open('','_blank','width=900,height=700');var html='<!DOCTYPE html><html><head><meta charset="utf-8"><title>Export</title><link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&family=IBM+Plex+Serif:ital,wght@0,400;0,500;0,600;0,700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet"><style>body{background:#fff;color:#0F172A;font-family:"IBM Plex Serif",Georgia,serif;line-height:1.75;padding:60px 80px;max-width:820px;margin:0 auto}h1{font-size:2em;font-weight:700;margin:1.2em 0 .5em;letter-spacing:-0.3px}h2{font-size:1.4em;font-weight:700;margin:1.5em 0 .4em;border-bottom:1px solid #e2e8f0;padding-bottom:.3em}p{margin:.8em 0}code{background:#f1f5f9;padding:.15em .4em;border-radius:3px;font-size:.88em;color:#16A34A}pre{background:#f8fafc;padding:1em;border-radius:6px;border:1px solid #e2e8f0}blockquote{border-left:3px solid #22C55E;margin:1em 0;padding:.3em 1em;color:#64748B;font-style:italic;background:#f0fdf4}table{width:100%;border-collapse:collapse}th,td{border:1px solid #e2e8f0;padding:.6em;text-align:left}th{background:#f8fafc;font-weight:600}a{color:#16A34A}img{max-width:100%}@media print{@page{margin:.5in}body{padding:20px}}.print-hint{background:#f0fdf4;border:1px solid #22C55E;padding:1rem;margin-bottom:1rem;border-radius:6px;color:#166534;font-family:"IBM Plex Sans",sans-serif}@media print{.print-hint{display:none!important}}</style></head><body><div class="print-hint"><strong>Export to PDF:</strong> In the print dialog, choose "Save as PDF" as the destination, then click Save.</div>'+hc+'</body></html>';pw.document.write(html);pw.document.close();pw.document.fonts.ready.then(function(){setTimeout(function(){pw.print()},800)});T('Select "Save as PDF" in print dialog')};
...
document.addEventListener('DOMContentLoaded',function(){var ed=$('editor'),ds=LD(),aid=localStorage.getItem(AT);if(aid){var d=ds.find(function(x){return x.id===aid});if(d){S.cd=d;ed.value=d.content}}else if(ds.length>0){S.cd=ds[0];ed.value=ds[0].content}else{ed.value='# Welcome to Mercury\\\\n\\\\nStart writing markdown here.\\\\n\\\\n- Type on the **left**.\\\\n- See the preview on the **right**.\\\\n- Press **Ctrl/Cmd + P** to hide preview.\\\\n- Press **Ctrl/Cmd + Shift + F** for focus mode.'}R();RDL();$('mode-indicator').textContent='SOURCE';ed.addEventListener('input',function(){R()});ed.addEventListener('keydown',function(e){var m=e.metaKey||e.ctrlKey;if(m&&e.key==='b'){e.preventDefault();WS('**','**')}if(m&&e.key==='i'){e.preventDefault();WS('*','*')}if(m&&e.shiftKey&&e.key==='F'){e.preventDefault();TF()}if(m&&e.key==='p'){e.preventDefault();TP()}});document.querySelectorAll('.editor-toolbar button[data-action]').forEach(function(b){b.addEventListener('click',function(){var a=this.dataset.action;if(a==='bold')WS('**','**');if(a==='italic')WS('*','*');if(a==='heading')IH();if(a==='link'){var u=prompt('URL:')||'';if(u)WS('[',']('+u+')')}if(a==='list'){var ed=$('editor'),ls=ed.value.lastIndexOf('\\\\n',ed.selectionStart-1)+1;ed.setSelectionRange(ls,ls);document.execCommand('insertText',false,'- ');R()}})});$('btn-toggle-preview').addEventListener('click',TP);$('btn-focus').addEventListener('click',TF);$('btn-pdf').addEventListener('click',EP);$('btn-copy').addEventListener('click',CH);$('btn-import').addEventListener('click',function(){$('file-input').click()});$('btn-close-preview').addEventListener('click',function(){S.po=false;$('preview-col').classList.add('hidden');$('preview-col').classList.remove('visible')});$('btn-new-doc').addEventListener('click',ND);var fi=$('file-input');fi.addEventListener('change',async function(){var f=fi.files[0];fi.value='';if(f)await IF(f)});var ew=$('editor-wrapper');ew.addEventListener('dragover',function(e){e.preventDefault()});ew.addEventListener('drop',async function(e){e.preventDefault();var f=e.dataTransfer.files[0];if(f)await IF(f)});setInterval(SD,5000)});
})();`;
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@300;400;500;600;700&family=IBM+Plex+Serif:ital,wght@0,400;0,500;0,600;0,700;1,400&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet">
...
<div id="app">
  ...
  <section id="editor-col">
    <div class="editor-toolbar">
      ...
      <button class="tb-mode" id="btn-toggle-mode" title="Source / WYSIWYG (Ctrl+/)">SOURCE</button>
    </div>
